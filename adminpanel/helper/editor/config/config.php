...
/*
|--------------------------------------------------------------------------
| Custom baseUrl for localhost
|--------------------------------------------------------------------------
 */
$isLocalhost = (in_array($_SERVER['REMOTE_ADDR'], array(
	'127.0.0.1',
	'::1'
)));
...
$baseUrl = pathUrl();
$pos = strpos($baseUrl, '/adminpanel/helper/editor');
if ($pos) $baseUrl = mb_substr($baseUrl, 0, $pos);
...
$config = array(
	...
	|
	*/
	'base_url' => $isLocalhost ? $baseUrl : ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] && ! in_array(strtolower($_SERVER['HTTPS']), array( 'off', 'no' ))) ? 'https' : 'http') . '://' . $_SERVER['HTTP_HOST'],
