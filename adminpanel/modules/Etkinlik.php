...
class Etkinlik  extends Settings{
    ...
    public function __construct($settings)
    {
        ...
        $this->tablist = array(
            array("title" => "Etkinlik Listesi", "href" => "liste", "icon" => "mdi mdi-view-sequential"),
    ...
    public function liste($id=null)
    {
        ...
        $text .= $pagelist->PageList(array(
            ...
            'page'=>$this->table,
            "place"=>"etkinlik adı giriniz",
            "showing"=>$showing,
            "toplamVeri"=>$toplamVeri,
            "search"=>true,
            'button' => array(array('title'=>'Etkinlik Ekle','href'=>$this->BaseAdminURL($this->modulName.'/ekle'),"class"=>"btn btn-success")),
            'p'=>array(
                array('dataTitle'=>'baslik', 'class'=>'sort'),
                array('dataTitle'=>'baslangic', 'class'=>'sort', "type"=>"date", "dateFormat"=>"d.m.Y H:i", "labelClass"=>"label label-default"),
                //array('dataTitle'=>'bitis', 'class'=>'sort', "type"=>"date", "dateFormat"=>"d.m.Y H:i", "labelClass"=>"label label-default"),
            ),
            ...
            'baslik'=>array(
                ...
                array('title'=>'Başlangıç Tarihi','width'=>'10%'),
                //array('title'=>'Bitiş Tarihi','width'=>'10%'),
    ...
    {
        $this->SayfaBaslik = 'Etkinlik '.(($id) ? "Düzenle" : "Ekle");
        $text = '';
        $tabForm = array();
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            ...
            $tabForm[$dil]['text']  .= $form->input(array('value'=>((isset($data[$dil]['yer']) ? $this->temizle($data[$dil]['yer']) :'')),'title'=>'Etkinlik Yeri','lang'=>$dil,'name'=>'yer'));
        ...
        $text .= $form->date(array("format"=>"DD.MM.YYYY H:mm","required"=>"true", 'value'=>((isset($data["tr"]['baslangic']) ? date("d.m.Y H:i",strtotime($data["tr"]['baslangic'])) : date('d.m.Y H:i'))),'title'=>'Başlangıç Tarihi ve Saati','name'=>'baslangic'));

        //$text .= $form->date(array("format"=>"DD.MM.YYYY H:mm","required"=>"true", 'value'=>((isset($data["tr"]['bitis']) ? date("d.m.Y H:i",strtotime($data["tr"]['bitis'])) : date('d.m.Y H:i'))),'title'=>'Bitiş Tarihi ve Saati','name'=>'bitis'));
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            ...
            if($dil == "tr"):
                $post[$dil] = array(
                    ...
                    'yer'=> $this->kirlet($this->_POST('yer',$dil)),
                    'ozet'=>$this->kirlet($this->_POST('ozet',$dil)),
                    'detay'=>$this->kirlet($this->_POST('detay',$dil)),
                    'baslangic'=>date('Y-m-d H:i:s', strtotime($this->_POST("baslangic"))),
                    'bitis'=>date('Y-m-d H:i:s', strtotime($this->_POST("bitis"))),
