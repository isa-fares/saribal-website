...
class Fuar  extends Settings{
    ...
    public function __construct($settings)
    {
        ...
        $this->tablist = array(
            array("title" => "Fuar Listesi", "href" => "liste", "icon" => "mdi mdi-view-sequential"),
            //array("title" => "Fuar Modül Ayarları", "href" => "settings", "icon" => "mdi mdi-settings")
        );
    ...
    public function liste($id=null)
    {
        ...
        $text .= $pagelist->PageList(array(
            ...
            "search"=>true,
            'button' => array(array('title'=>'Fuar Ekle','href'=>$this->BaseAdminURL($this->modulName.'/ekle'),"class"=>"btn btn-success")),
            'p'=>array(
    ...
    {
        $this->SayfaBaslik = 'Fuar '.(($id) ? "Düzenle" : "Ekle");
        $text = '';
        $tabForm = array();
        ...
        $text .= $form->date(array("format"=>"DD.MM.YYYY","required"=>"true", 'value'=>((isset($data["tr"]['baslangic']) ? date("d.m.Y",strtotime($data["tr"]['baslangic'])) : date('d.m.Y'))),'title'=>'Başlangıç Tarihi','name'=>'baslangic'));

        $text .= $form->date(array("format"=>"DD.MM.YYYY","required"=>"true", 'value'=>((isset($data["tr"]['bitis']) ? date("d.m.Y",strtotime($data["tr"]['bitis'])) : date('d.m.Y'))),'title'=>'Bitiş Tarihi','name'=>'bitis'));
