...
class Icerikler  extends Settings{
    ...
    public function __construct($settings)
    {
        ...
        $this->tablist = array(
            array("title" => "İçerik Listesi", "href" => "liste", "icon" => "mdi mdi-view-sequential"),
    ...
    public function liste($id=null)
    {
        ...
        $text .= $pagelist->PageList(array(
            ...
            "search"=>true,
            'button' => array(array('title'=>'İçerik Ekle','href'=>$this->BaseAdminURL($this->modulName.'/ekle'),"class"=>"btn btn-success")),
            'p'=>array(
    ...
    {
        $this->SayfaBaslik = 'İçerik '.(($id) ? "Düzenle" : "Ekle");
        $text = '';
        $tabForm = array();
       ...
       $text.= $form->file2(array('url'=>$this->BaseURL('upload')."/".$this->modulName,'folder'=>$this->modulName,'title'=>'Icon','name'=>'resim','resimBoyut'=>$this->modul_image_size($this->modul_info["id"]),'src'=>((isset($data['tr']['resim'])) ? $data['tr']['resim'] :'')));
        ...
        $text .= $form->checkbox(array('value'=>((isset($data["tr"]['standart'])) ? $this->temizle($data["tr"]['standart']) :''),'title'=>'Evet','name'=>'standart','help'=>'Standart Özellik mi?', "checked"=>((isset($data["tr"]["standart"])) ? "" : "checked")));
