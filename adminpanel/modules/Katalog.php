...
class Katalog  extends Settings{
    ...
    public function __construct($settings)
    {
        ...
        $this->tablist = array(
            ...
            //array("title" => "Katalog Modül Ayarları", "href" => "settings", "icon" => "mdi mdi-settings")
        );
    ...
    public function liste($id=null)
    {
        ...
        $text .= $pagelist->PageList(array(
            ...
            "resim"=>true,
            "search"=>true,
            'button' => array(array('title'=>'Bülten Ekle','href'=>$this->BaseAdminURL($this->modulName.'/ekle'),"class"=>"btn btn-success")),
            'p'=>array(
                array('dataTitle'=>'baslik', 'class'=>'sort'),
                //array('dataTitle'=>'boyut', 'class'=>'sort'),
            ),
            ...
            'buton'=> array(
                //array("disable_delete"=>true,'type'=>'button2','align'=>"center", 'title'=>'Sayfa Yükle','class'=>'btn bg-olive','dataname'=>'fotoekle','url'=>$this->BaseAdminURL('Resim/fotoekle/'), "data-icon"=>"ti-camera", "modul"=>$this->table),
            ...
            'baslik'=>array(
                ...
                //array('title'=>'Sayfa Boyutu','width'=>'5%'),
                //array('title'=>'Sayfalar','width'=>'5%', "align"=>"center"),
    ...
    public function ekle($id=null)
    {
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            ...
            //$tabForm[$dil]['text']  .= $form->input(array("required"=>array("tr"), 'value'=>((isset($data[$dil]['boyut']) ? $this->temizle($data[$dil]['boyut']) :'')),'title'=>'Resim Boyutları','lang'=>$dil,'name'=>'boyut', "help"=>"Örneğin (800x1200)"));
        ...
        $fileUpload = $modal->fileLoad(['name'=>'files','params' => array('modul'=>$this->modulName,'folder'=>'../'.$this->settings->config('folder').$this->modulName.'/','baslik'=>'baslik_tr','id'=>$id),'sql'=>(($id) ? $this->dbConn->sorgu("select * from dosyalar where type='{$this->modulName}' and data_id='$id' and tur = 'dosya' and sil <> 1 ORDER   BY  sira"):array())]);
        $text .= $modal->infoform(array('title'=>'Pdf Dosyası','govde'=>$fileUpload));
        ...
        $text.= $form->file(array('url'=>$this->BaseURL('upload')."/".$this->modulName,'folder'=>$this->modulName,'title'=>'Kapak Resmi','name'=>'resim','resimBoyut'=>$this->modul_image_size($this->modul_info["id"]),'src'=>((isset($data['tr']['resim'])) ? $data['tr']['resim'] :'')));
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            ...
            if($dil == "tr"):
                $post[$dil] = array(
                    ...
                    'boyut'=> $this->kirlet($this->_POST('boyut',$dil)),
