...
class Kurum  extends Settings{
    ...
    public function __construct($settings)
    {
        ...
        $this->tablist = array(
            array("title" => "Kurumlar Listesi", "href" => "liste", "icon" => "mdi mdi-view-sequential"),
            //array("title" => "Belge Grupları", "href" => "kategoriler", "icon" => "mdi mdi-ungroup"),
    ...
    public function liste($id=null)
    {
        ...
        $this->SayfaBaslik = "Kurum Listesi";
        ...
        $text .= $pagelist->PageList(array(
            //'flitpage' => array("url"=>$filterURL,"title"=>"Grup Seçiniz",'sql'=>"select * from $this->ktable WHERE sil <> 1 and  ",'option'=>array('value'=>'id','title'=>'baslik'), "name"=>"sayfaFilter", "class"=>"flit_filter", "kat"=>"ustu"),
            'id'=>$id,
            'modul_id'=>$this->modul_info["id"],
            'modul_name'=>$this->modulName,
            'page'=>$this->table,
            'resim'=>true,
            "place"=>"kurumlarda ara",
            "showing"=>$showing,
            "toplamVeri"=>$toplamVeri,
            "search"=>true,
            'button' => array(array('title'=>'Kurum Ekle','href'=>$this->BaseAdminURL($this->modulName.'/ekle&kid='.$id),"class"=>"btn btn-success")),
            'p'=>array(
            ...
            'baslik'=>array(
                array('title'=>'Kurum Adı','width'=>'20%'),
                //array('title'=>'Grup','width'=>'20%'),
    ...
    public function ekle($id=null)
    {
        ...
        $this->SayfaBaslik = 'Kurum '.(($id) ? "Düzenle" : "Ekle");
        $text = '';
        $tabForm = array();
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            $tabForm[$dil]['text']  = $form->input(array('required'=>array("tr"), 'value'=>((isset($data[$dil]['baslik']) ? $this->temizle($data[$dil]['baslik']) :'')),'title'=>'Kurum Adı','lang'=>$dil,'name'=>'baslik'));
            $tabForm[$dil]['text']  .= $form->input(array("icon"=>"fa fa-link", 'value'=>((isset($data[$dil]['link']) ? $this->temizle($data[$dil]['link']) :'')),'title'=>'Kurum Websitesi Adresi','lang'=>$dil,'name'=>'link'));
        ...
        //$text .= $form->select(array('required'=>true,'title'=>'Belge Grubu','name'=>'kid','data'=> $form->parent(array('sql'=>"select * from $this->ktable WHERE sil <> 1",'option'=>array('value'=>'id','title'=>'baslik'),'selected'=>((isset($data['tr']['kid'])) ? $data['tr']['kid'] : (($request_kid != 0) ? $request_kid : ""))),0,0)));
        ...
        $text.= $form->file(array('url'=>$this->BaseURL('upload')."/".$this->modulName,'folder'=>$this->modulName,'title'=>'Kurum Logosu','name'=>'SayfaResim','resimBoyut'=>$this->modul_image_size($this->modul_info["id"]),'src'=>((isset($data['tr']['resim'])) ? $data['tr']['resim'] :'')));
    ...
    public function kategoriler($id=null)
    {
        ...
        $this->SayfaBaslik = "Belge Grupları Listesi";
        ...
        $text .= $pagelist->KategoriList(array(
            ...
            "toplamVeri"=>$toplamVeri,
            "search"=>true,
            'button' => array(array('title'=>'Belge Grubu Ekle','href'=>$this->BaseAdminURL($this->modulName.'/kategoriEkle'),"class"=>"btn btn-success")),
            'p'=>array(
                array('dataTitle'=>'baslik', 'class'=>'sort'),
            ),
    ...
    {
        $this->SayfaBaslik = 'Grup '.(($id) ? "Düzenle" : "Ekle");
        $text = '';
        $tabForm = array();
        $form = new Form($this->settings);
        $tabs = new Tabs($this->settings);
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            ...
            //$tabForm[$dil]['text'] .= $form->textarea(array('value'=>((isset($data[$dil]['detay']) ? $this->temizle($data[$dil]['detay']) :'')),'title'=>'Detay','name'=>'detay','lang'=>$dil));
        endforeach;
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            ...
            if($dil == "tr"):
                $post[$dil] = array(
                    ...
                    'link'=> $this->kirlet($this->_POST('link',$dil)),
                    'aktif'=> ($this->_POST('aktif')) ? $this->_POST('aktif'):0,
                    'resim' => $this->_RESIM_BASE64('kategoriResim', $this->modulName),
                    'dil' => $dil
                );
            else:
                $post[$dil] = array(
