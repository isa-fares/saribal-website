...
class Marka  extends Settings{
    ...
    public function __construct($settings)
    {
        ...
        $this->tablist = array(
            array("title" => "Dil Listesi", "href" => "liste", "icon" => "mdi mdi-view-sequential"),
            //array("title" => "Markalar Modül Ayarları", "href" => "settings", "icon" => "mdi mdi-settings")
        );
    ...
    public function liste($id=null)
    {
        ...
        $text .= $pagelist->PageList(array(
            ...
            "search"=>true,
            'button' => array(array('title'=>'Dil Ekle','href'=>$this->BaseAdminURL($this->modulName.'/ekle'),"class"=>"btn btn-success")),
            'p'=>array(
            ...
            'baslik'=>array(
                ...
                array('title'=>'Fiyat','width'=>'5%', "align"=>"center"),
    ...
    {
        $this->SayfaBaslik = 'Dil '.(($id) ? "Düzenle" : "Ekle");
        $text = '';
        $tabForm = array();
        ...
        $text.= $form->file2(array('url'=>$this->BaseURL('upload')."/".$this->modulName,'folder'=>$this->modulName,'title'=>'Resim','name'=>'resim','resimBoyut'=>$this->modul_image_size($this->modul_info["id"]),'src'=>((isset($data['tr']['resim'])) ? $data['tr']['resim'] :'')));

        $text.=$form->openBox().$form->openBoxBody();

        $text .= $form->input(array("required"=>true,"icon"=>"fa fa-try", "required"=>true,'value'=>((isset($data["tr"]['fiyat']) ? $this->moneyFormat($data["tr"]['fiyat']) :'')),'title'=>'Fiyatı','name'=>'fiyat'));


        $text .= $form->input(array("required"=>array("tr"),"help"=>"Örneğin: tr",'value'=>((isset($data["tr"]['kod']) ? $this->temizle($data["tr"]['kod']) :'')),'title'=>'Dil Kodu','name'=>'kod'));

        $text .= $form->checkbox(array('value'=>((isset($data["tr"]['sabit'])) ? $this->temizle($data["tr"]['sabit']) :''),'title'=>'Evet','name'=>'sabit','help'=>'Sabit Seçenek mi?', "checked"=>((isset($data["tr"]["sabit"])) ? "" : "checked")));


        $text .= $form->checkbox(array('value'=>((isset($data["tr"]['aktif'])) ? $this->temizle($data["tr"]['aktif']) :''),'title'=>'Aktif','name'=>'aktif','help'=>'Onay Durumu', "checked"=>((isset($data["tr"]["aktif"])) ? "" : "")));
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            ...
            if($dil == "tr"):
                $post[$dil] = array(
                    ...
                    'sabit'=> ($this->_POST('sabit')) ? $this->_POST('sabit'):0,
                    'dil' => $dil
                );
            else:
                $post[$dil] = array(
