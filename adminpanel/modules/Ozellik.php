...
class Ozellik  extends Settings{
    ...
    public function __construct($settings)
    {
        ...
        $this->tablist = array(
            array("title" => "Özellikler Listesi", "href" => "liste", "icon" => "mdi mdi-view-sequential"),
    ...
    public function liste($id=null)
    {
        ...
        $text .= $pagelist->PageList(array(
            ...
            "search"=>true,
            'button' => array(array('title'=>'Özellik Ekle','href'=>$this->BaseAdminURL($this->modulName.'/ekle'),"class"=>"btn btn-success")),
            'p'=>array(
    ...
    {
        $this->SayfaBaslik = 'Özellik '.(($id) ? "Düzenle" : "Ekle");
        $text = '';
        $tabForm = array();
        ...
        $text.= $form->file2(array('url'=>$this->BaseURL('upload')."/".$this->modulName,'folder'=>$this->modulName,'title'=>'Icon','name'=>'resim','resimBoyut'=>$this->modul_image_size($this->modul_info["id"]),'src'=>((isset($data['tr']['resim'])) ? $data['tr']['resim'] :'')));
        ...
        $text .= $form->select(array('title'=>'İkon ve Zemin Rengi','name'=>'renk','data'=> $form->parent(array('array'=>array(
            array('id'=>'red','text'=> 'Kırmızı'),
            array('id'=>'blue','text'=>'Mavi'),
            array('id'=>'yellow','text'=>'Turuncu'),
            array('id'=>'green','text'=>'Yeşil'),
            array('id'=>'ash','text'=>'Antrasit'),
        ),
        'option'=>array('value'=>'id','title'=>'text'),'selected'=>$data["tr"]["renk"]),0,0)));

        $text .= $form->checkbox(array('value'=>((isset($data["tr"]['standart'])) ? $this->temizle($data["tr"]['standart']) :''),'title'=>'Evet','name'=>'standart','help'=>'Standart Özellik mi?', "checked"=>((isset($data["tr"]["standart"])) ? "" : "")));
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            ...
            if($dil == "tr"):
                $post[$dil] = array(
                    ...
                    'standart'=> ($this->_POST('standart')) ? $this->_POST('standart'):0,
