...
class Popup  extends Settings{
    ...
    public function __construct($settings)
    {
        ...
        $this->tablist = array(
            array("title" => "Popup Listesi", "href" => "liste", "icon" => "mdi mdi-view-sequential"),
    ...
    public function liste($id=null)
    {
        ...
        $text .= $pagelist->PageList(array(
            ...
            "search"=>true,
            'button' => array(array('title'=>'Popup Ekle','href'=>$this->BaseAdminURL($this->modulName.'/ekle'),"class"=>"btn btn-success")),
            'p'=>array(
                array('dataTitle'=>'baslik', 'class'=>'sort'),
                array('dataTitle'=>'tarih', "type"=>"date", "dateFormat"=>"d-m-Y H:i"),
            ),
            ...
            'baslik'=>array(
                ...
                array('title'=>'Bitiş Tarihi','width'=>'10%', "align"=>"center"),
    ...
    {
        $this->SayfaBaslik = 'Popup '.(($id) ? "Düzenle" : "Ekle");
        $text = '';
        $tabForm = array();
        ...
        $text .= $form->input(array('yardim'=>"", 'icon'=>'fa fa-unlink','value'=>((isset($data["tr"]['link']) ? $this->temizle($data["tr"]['link']) :'')),'title'=>'Url','name'=>'link', 'help'=>"Http dahil full url giriniz. Örneğin: ".$this->baseURL("iletisim.html")));

        $text .= $form->date(array("format"=>"DD.MM.YYYY HH:mm","required"=>"true", 'value'=>((isset($data["tr"]['tarih']) ? date("d.m.Y H:i",strtotime($data["tr"]['tarih'])) : date('d.m.Y H:i'))),'title'=>'Son Geçerlilik Tarihi','name'=>'tarih'));
        ...
        $text.= $form->file2(array('url'=>$this->BaseURL('upload')."/".$this->modulName,'folder'=>$this->modulName,'title'=>'Popup Görseli','name'=>'resim','resimBoyut'=>$this->modul_image_size($this->modul_info["id"]),'src'=>((isset($data['tr']['resim'])) ? $data['tr']['resim'] :'')));
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            ...
            if($dil == "tr"):
                $post[$dil] = array(
                    ...
                    'tarih'=>date('Y-m-d H:i:s', strtotime($this->_POST("tarih"))),
