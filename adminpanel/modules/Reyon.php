...
class Reyon  extends Settings{
    ...
    public function __construct($settings)
    {
        ...
        $this->tablist = array(
            array("title" => "Reyon Listesi", "href" => "liste", "icon" => "mdi mdi-view-sequential"),

        );

        if ($this->user_type == 1){
            array_push($this->tablist, array("title" => "Reyon Modül Ayarları", "href" => "settings", "icon" => "mdi mdi-settings"));
        }
    ...
    public function liste($id=null)
    {
        ...
        $text .= $pagelist->PageList(array(
            ...
            "search"=>true,
            'button' => array(array('title'=>'Reyon Ekle','href'=>$this->BaseAdminURL($this->modulName.'/ekle'),"class"=>"btn btn-success")),
            'p'=>array(
    ...
    {
        $this->SayfaBaslik = 'Reyon '.(($id) ? "Düzenle" : "Ekle");
        $text = '';
        $tabForm = array();
        ...
        $text.= $form->file(array('url'=>$this->BaseURL('upload')."/".$this->modulName,'folder'=>$this->modulName,'title'=>'Reyon Kapak Görseli','name'=>'resim','resimBoyut'=>$this->modul_image_size($this->modul_info["id"]),'src'=>((isset($data['tr']['resim'])) ? $data['tr']['resim'] :'')));

        $text.= $form->file2(array('url'=>$this->BaseURL('upload')."/".$this->modulName,'folder'=>$this->modulName,'title'=>'Reyon Iconu','name'=>'icon','resimBoyut'=>$this->modul_image_size($this->modul_info["id"], "thumb"),'src'=>((isset($data['tr']['icon'])) ? $data['tr']['icon'] :'')));
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            ...
            if($dil == "tr"):
                $post[$dil] = array(
                    ...
                    'icon' => $this->_RESIM_BASE64('icon', $this->modulName),
