...
class Yemek  extends Settings{
    ...
    public function liste($id=null)
    {
        ...
        $text .= $pagelist->PageList(array(
            ...
            "search"=>true,
            'button' => array(array('title'=>'Tarif Ekle','href'=>$this->BaseAdminURL($this->modulName.'/ekle'),"class"=>"btn btn-success")),
            'p'=>array(
    ...
    {
        $this->SayfaBaslik = 'Tarif '.(($id) ? "Düzenle" : "Ekle");
        $text = '';
        $tabForm = array();
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            ...
            $tabForm[$dil]['text'] .= $form->textEditor(array('value'=>((isset($data[$dil]['malzemeler']) ? $this->temizle($data[$dil]['malzemeler']) :'')),'title'=>'Malzemeler','name'=>'malzemeler','lang'=>$dil,'height' => '230'));

            $tabForm[$dil]['text'] .= $form->textEditor(array('value'=>((isset($data[$dil]['detay']) ? $this->temizle($data[$dil]['detay']) :'')),'title'=>'Hazırlanış','name'=>'detay','lang'=>$dil,'height' => '230'));
        endforeach;
        ...
        $text.= $form->file(array('url'=>$this->BaseURL('upload')."/".$this->modulName,'folder'=>$this->modulName,'title'=>'Tarif Görseli','name'=>'resim','resimBoyut'=>$this->modul_image_size($this->modul_info["id"]),'src'=>((isset($data['tr']['resim'])) ? $data['tr']['resim'] :'')));

        $text.=$form->openBox().$form->openBoxBody();

        $text.= $form->input(array('value'=>((isset($data["tr"]['hazirlanma']) ? $this->temizle($data["tr"]['hazirlanma']) :'')),'title'=>'Hazırlanma Süresi','name'=>'hazirlanma', "help"=>"Dakika"));


        $text.= $form->input(array('value'=>((isset($data["tr"]['pisirme']) ? $this->temizle($data["tr"]['pisirme']) :'')),'title'=>'Pişirme Süresi','name'=>'pisirme', "help"=>"Dakika"));

        $text.= $form->input(array('value'=>((isset($data["tr"]['kisi']) ? $this->temizle($data["tr"]['kisi']) :'')),'title'=>'Kaç Kişilik','name'=>'kisi',));
        ...
        foreach ($this->settings->lang('lang') as $dil=>$title):
            ...
            if($dil == "tr"):
                $post[$dil] = array(
                    ...
                    'hazirlanma'=> $this->kirlet($this->_POST('hazirlanma')),
                    'kisi'=> $this->kirlet($this->_POST('kisi')),
                    'pisirme'=> $this->kirlet($this->_POST('pisirme')),
                    'malzemeler'=> $this->kirlet($this->_POST('malzemeler',$dil)),
