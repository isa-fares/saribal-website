...
class AppCropper
{
    ...
    public function run()
    {
        if (!isset($this->name)) throw new Exception("Input ismi belirtilmedi");
        if (!isset($this->uniqueId)) {
            var_dump($this->uniqueId);
            $this->uniqueId = uniqid('cropper_'); // set uniqueId if its empty
        }

        $this->registerInputLabel();
        $this->registerCropperOptions();
        ...
        $file = __DIR__ . '/Template/app-cropper/cropper.php';

        $this->require_with($file, $params);
    }

    /**
     * @throws Exception
     */
    private function registerCropperOptions()
    {
        $options = $this->cropperOptions;
        if (!isset($options['width']) && !isset($options['height'])) {
            throw new Exception('"genişlik" ve "yükseklik" belirtilmesi zorunludur.');
        }
    ...
    private function registerInputLabel()
    {
        $label = $this->label;
    ...
    /*
    function render($name, $parameters) {
        // Path to templates
    ...
    protected function require_with($file, $params)
    {
        extract($params);
